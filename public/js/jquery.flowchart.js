...
jQuery(function ($) {
    ...
    $.widget("flowchart.flowchart", {
        ...
        _getOperatorFullElement: function (operatorData) {
            ...
            var $operator_select_group = $('<select class="form-control"><option></option></select>');
            var $operator_customer_default=$('<input class="form-control" readonly disable value="مشتری"/>');
            $.each(operatorData.appendSelect, function (index, value) {
                var o = new Option(value.NAME, value.ID);
                $operator_select_group.append(o);
            });



            $operator_select_group.change(function () {
                operatorData.groupSelected = $(this).val();
            });
            $operator_select_group.val(operatorData.groupSelected).change();
            if (operatorData.appendSelect.length > 0) {
                infos.body = $operator_select_group;
            }
            else{
                infos.body =$operator_customer_default;
            }
